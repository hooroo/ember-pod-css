...
BrocComponentCssPreprocessor.prototype.write = function (readTree, destDir) {
  ...
  var classNameCache = this.classNameCache;
  ...
  return readTree(this.inputTree).then(function(srcDir) {
    ...
    for (var i = 0, l = paths.length; i < l; i++) {
      ...
      if (filepath[filepath.length-1] !== '/') {
        ...
        if (podClassName in classNameCache) {
          className = classNameCache[podClassName];
        } else {
          className = classNameCache[podClassName] = podClassName
        }

        var fileContents = fs.readFileSync(path.join(srcDir, filepath)).toString();

        buffer.push(processors[pod.extension](fileContents, className));
        pod.lookup[podPath.join('/')] = className;
      }
    }
